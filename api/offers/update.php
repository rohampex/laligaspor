...
$stmt = $pdo->prepare('UPDATE offers SET title = ?, description = ?, discount_percent = ?, image_url = ?, product_ids = ?, start_at = ?, end_at = ?, is_active = ?, updated_at = NOW() WHERE id = ?');
$stmt->execute([$title, $description, $discountPercent, $imageUrl, json_encode($productIds, JSON_UNESCAPED_UNICODE), $startAt, $endAt, $isActive, $id]);
logSecurityEvent($pdo, $_SESSION['admin_id'], 'offer_update', "Offer {$id} updated");
