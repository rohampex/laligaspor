...
$stmt = $pdo->prepare('UPDATE products SET name = ?, slug = ?, description = ?, short_description = ?, category_id = ?, price = ?, discount_price = ?, discount_percent = ?, sku = ?, barcode = ?, brand = ?, team = ?, season = ?, is_featured = ?, is_active = ?, updated_at = NOW() WHERE id = ?');
$stmt->execute([$name, $slug, $description, $shortDescription, $categoryId, $price, $discountPrice, $discountPercent, $sku, $barcode, $brand, $team, $season, $isFeatured, $isActive, $id]);
...
foreach ($productImages as $sort => $imagePath) {
    ...
    $stmt->execute([$id, $imagePath, $sort, $sort === 0 ? 1 : 0]);
}
...
foreach ($sizes as $item) {
    ...
    $stmt->execute([$id, $size, $stockValue, $available]);
}
logSecurityEvent($pdo, $_SESSION['admin_id'], 'product_update', "Product {$id} updated: {$name}");
